...
class SCoGANTrainer(object):
    ...
    def gram_matrix(self,input):
        ...
        num_locations = input.shape[1] * input.shape[2] * input.shape[3]
    ...
    def StyleContentLoss(self, style_fake, style_target, content_fake1, content_fake2, args):
        ...
        content_loss = tf.add_n([tf.reduce_mean(((content_fake1[name] - content_fake2[name])**2)/(content_fake1[name].shape[1]*content_fake1[name].shape[2]*content_fake1[name].shape[3])) for name in content_fake1.keys()])
